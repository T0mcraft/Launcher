...
public class GameUpdater implements Runnable
{
	...
	protected String[] genericErrorMessage = { "Une erreur s'est produite lors du chargement de l'applet.",
			"S'il vous plaît contacter le support pour résoudre ce problème.", "<placeholder for error message>" };
	protected boolean certificateRefused;
	protected String[] certificateRefusedMessage = { "Accès à l'applet refusé.", "Acceptez, s'il vous plaît, le texte des autorisations pour permettre",
			"à l'applet de poursuivre le processus de chargement." };
	...
	protected void loadJarURLs() throws Exception
	{
		...
		for (int i = 0; i < jarCount - 1; i++)
		{
			...
			{
				path = new URL("http://dl.nukkit-project.fr/current/");
			}

			System.out.println(path + nextToken.replaceAll("minecraft.jar", "nukkit.jar"));
			if (nextToken.indexOf("minecraft.jar") >= 0)
			{
				this.urlList[i] = new URL(path, nextToken.replaceAll("minecraft.jar", "nukkit.jar"));
			}
			else
			{
	...
	public void run()
	{
		...
		try
		{
			...
			if (this.latestVersion != null)
			{
				...
				{
					String version = "";
					URL url_version = new URL("http://dl.nukkit-project.fr/version.txt");
					try
					{
	...
	protected static void validateCertificateChain(Certificate[] ownCerts, Certificate[] native_certs) throws Exception
	{
		...
		for (int i = 0; i < ownCerts.length; i++)
			if (!ownCerts[i].equals(native_certs[i]))
				throw new Exception("Certificat différents: " + ownCerts[i] + " != " + native_certs[i]);
	}
