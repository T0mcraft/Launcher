...
public class Util
{
	...
	{
		if (workDir == null)
			workDir = getWorkingDirectory("Nukkit");
		return workDir;
	}
	...
	public static File getWorkingDirectory(String applicationName)
	{
		...
		if ((!workingDirectory.exists()) && (!workingDirectory.mkdirs()))
			throw new RuntimeException("Le dossier de destination n'a pas pu être créé : " + workingDirectory);
	...
	public static String excutePost(String targetURL, String urlParameters)
	{
		...
		try
		{
			...
			for (int i = 0; i < data.length; i++)
			{
				if (data[i] == bytes[i])
					continue;
				throw new RuntimeException("Les clÃ©s publics sont diffÃ©rentes");
			}
